...
public class BinTarget extends FaceTarget<RenderableToBin> {
	...
	// private final Map<VectorXYZ, Integer> vertexIndexMap = new
	// HashMap<VectorXYZ, Integer>();
	// private final Map<VectorXYZ, Integer> normalsIndexMap = new
	// HashMap<VectorXYZ, Integer>();
	// private final Map<VectorXZ, Integer> texCoordsIndexMap = new
	// HashMap<VectorXZ, Integer>();
	// private final Map<Material, String> materialMap = new HashMap<Material,
	// String>();
	...
	public void finish() {
		...
		// AttributeData[] ad = new AttributeData[0];
		// ad[0] = new AttributeData("", materialName, precision, values);
		...
		Mesh m = new Mesh(vertices, normals, indices, new AttributeData[0], new AttributeData[0]);
		try {
			this.ctmWriter.encode(m, "comment");
		} catch (IOException e) {
			e.printStackTrace();
	...
	public void drawTriangle(Material material, TriangleXYZ triangle,
		...
		for (int i = 0, n = vs.size(); i < n; i++) {
			...
			VectorXYZ coord = vs.get(i);

			VectorXYZ clampedCoord = new VectorXYZ(
					Math.round(coord.x * COORD_PRECISION) / COORD_PRECISION,
					Math.round(coord.y * COORD_PRECISION) / COORD_PRECISION,
					Math.round(coord.z * COORD_PRECISION) / COORD_PRECISION);

			vertex.position = clampedCoord;
			vertex.normal = clampedNormal;
			// vertex.material =
